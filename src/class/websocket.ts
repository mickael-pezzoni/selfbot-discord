...
export class WebSocketClient {
  ...
  opCodeByImpl: Partial<
    Record<
      OpCode,
      (data: DiscordWsEvent<null>) => DiscordWsEventImpl<unknown> | undefined
    >
  > = {
    0: undefined,
    1: undefined,
  ...
  discordEventNameByImpl: Record<
    EventName,
    | ((
        data: DiscordWsEvent<EventName>,
      ) => Promise<DiscordWsEventImpl<unknown> | void>)
    | undefined
  > = {
    READY: undefined,
    MESSAGE_CREATE: async (data: DiscordWsEvent<EventName>) => {
      ...
      const msgEvent = new DiscordMsgEvent(
        (data as DiscordWsEvent<"MESSAGE_CREATE">).d,
        this.auth,
        messageFunction,
      );
    ...
    MESSAGE_REACTION_ADD: async (data: DiscordWsEvent<EventName>) => {
      const reactFunction =
        this.#functionByParamDiscordEvent.MESSAGE_REACTION_ADD;
      if (!reactFunction) return;
      const reactEvent = new DiscordReactEvent(
        (data as DiscordWsEvent<"MESSAGE_REACTION_ADD">).d,
        reactFunction,
        this.auth,
      );
  ...
  listenEvent(): void {
    this.ws.addEventListener("open", (ev) => {
      ...
      this.ws.addEventListener("message", async (event) => {
        ...
        const wsData: Omit<DiscordWsEvent<never>, "d"> & { d: unknown } =
          JSON.parse(event.data.toString());

        const wsDataTyped = wsData as DiscordWsEvent<EventName | null>;
        if (wsDataTyped.t !== null) {
          const discordEventImpl = await this.discordEventNameByImpl[
            wsDataTyped.t
          ]?.(wsDataTyped as DiscordWsEvent<EventName>);
          this.#queue.add(
            () => discordEventImpl?.execute() ?? Promise.resolve(),
          );
          return;
        }

        const opCodeDiscordEvent = this.opCodeByImpl[wsData.op]?.(
          wsDataTyped as DiscordWsEvent<null>,
        );
